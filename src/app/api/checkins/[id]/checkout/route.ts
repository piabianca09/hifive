  ...
  try {
    ...
    if (existingLog.pass_type !== 'walk-in') {
      return NextResponse.json(
        { error: 'Only walk-in customers can be checked out this way' },
        { status: 400 }
      );
    }
    ...
    const hourlyRate =
      hourlyRates[existingLog.membership_type as string]?.[existingLog.pass_type as string] || 30;
    ...
    if (updateError) {
      ...
      return NextResponse.json(
        { error: 'Failed to update check-out time', details: updateError.message },
        { status: 500 }
      );
    }
